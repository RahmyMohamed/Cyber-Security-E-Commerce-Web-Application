...
app.post('/api/quiz/submit', checkAuth, async (req, res) => {
    ...
    if (parseInt(score) === parseInt(totalQuestions)) {
        try {
            ...
            const n8nCertificateUrl = process.env.N8N_WEBHOOK_URL || 'http://localhost:5678/webhook/ecommerce-order';
...
app.post('/api/checkout-mock', async (req, res) => {
    ...
    try {
        const n8nWebhookUrl = process.env.N8N_WEBHOOK_URL || 'http://localhost:5678/webhook/ecommerce-order';
